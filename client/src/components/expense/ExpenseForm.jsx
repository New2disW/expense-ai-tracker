...
import { Loader2, Sparkles, Save, Upload, FileText, AlertCircle, Wand2 } from "lucide-react";
...
export default function ExpenseForm({ expense, onSubmit, onCancel, isLoading }) {
  ...
  const handleFileUpload = (e) => {
    ...
    const worker = new Worker(new URL('../../workers/receiptWorker.js', import.meta.url), { type: 'module' });
    ...
    worker.onmessage = (event) => {
      ...
      if (success) {
        const missing = [];
        if (data.amount !== null && data.amount !== undefined) handleInputChange('amount', data.amount); else missing.push('amount');
        if (data.description) handleInputChange('description', data.description); else missing.push('description');
        if (data.category) handleInputChange('category', data.category); else missing.push('category');
        if (data.date) handleInputChange('date', data.date); else missing.push('date');
        setMissingFields(missing);
      } else {
        setUploadError(error || "Failed to parse receipt");
      }
      worker.terminate();
    };
  ...
  const handleNLSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${aiServiceUrl}/api/chat/parse-nl`, {
        method: 'POST',
        headers: {
        ...
        <div className="mb-6 p-4 rounded-lg bg-indigo-900/20 border border-indigo-500/30">
          ...
          <div className="flex gap-2">
            <Input
              ...
              placeholder="e.g. 'Spent $15 on coffee at Starbucks today'"
            ...
            >
              {isParsingNL ? <Loader2 className="w-4 h-4 animate-spin" /> : "Magic Fill"}
            </Button>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label htmlFor="amount" className="text-sm font-bold text-slate-300">
                Amount ($) {missingFields.includes('amount') && <span className="text-amber-400 text-xs ml-2">- Please verify</span>}
              </Label>
              <Input
                id="amount"
                type="number"
            ...
            <div className="space-y-2">
              <Label htmlFor="date" className="text-sm font-bold text-slate-300">
                Date {missingFields.includes('date') && <span className="text-amber-400 text-xs ml-2">- Please verify</span>}
              </Label>
              <Input
                id="date"
                type="date"
          ...
          <div className="space-y-2">
            <Label htmlFor="description" className="text-sm font-bold text-slate-300">
              Description {missingFields.includes('description') && <span className="text-amber-400 text-xs ml-2">- Please verify</span>}
            </Label>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label htmlFor="category" className="text-sm font-bold text-slate-300">
                Category {missingFields.includes('category') && <span className="text-amber-400 text-xs ml-2">- Please verify</span>}
              </Label>
              <Select
